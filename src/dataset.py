...
class TimeSeriesDataset:
    ...
    def _process_df(self, keep_ratio):    
        self.df = add_all_ta_features(self.df, close="Adj Close", high="High", low="Low", open="Open", volume="Volume")
    ...
    def _preprocess(self):
        ...
        self.X = np.reshape(self.X, (self.X.shape[0] * self.X.shape[1], self.X.shape[2])) # with shape (n_samples * time_steps, n_features)
        self.X = self.scaler.fit_transform(self.X)
        self.X = np.reshape(self.X, (self.X.shape[0] // self.time_steps, self.time_steps, self.X.shape[1]))
